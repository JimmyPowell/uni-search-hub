...
func InitRedis(config config.RedisConfig) {
	if config.Addr == "" {
		RedisEnabled = false
		utils.SysLog("REDIS_CONN_STRING not set, Redis is not enabled")
		return
	}
...
func RedisSet(key string, value string, expiration time.Duration) error {
	if common.DebugEnabled {
		utils.SysLog(fmt.Sprintf("Redis SET: key=%s, value=%s, expiration=%v", key, value, expiration))
	}
	ctx := context.Background()
...
func RedisGet(key string) (string, error) {
	if common.DebugEnabled {
		utils.SysLog(fmt.Sprintf("Redis GET: key=%s", key))
	}
	ctx := context.Background()
...
func RedisDelKey(key string) error {
	if common.DebugEnabled {
		utils.SysLog(fmt.Sprintf("Redis DEL Key: key=%s", key))
	}
	ctx := context.Background()
...
func RedisHSetObj(key string, obj interface{}, expiration time.Duration) error {
	if common.DebugEnabled {
		utils.SysLog(fmt.Sprintf("Redis HSET: key=%s, obj=%+v, expiration=%v", key, obj, expiration))
	}
	ctx := context.Background()
...
func RedisHGetObj(key string, obj interface{}) error {
	if common.DebugEnabled {
		utils.SysLog(fmt.Sprintf("Redis HGETALL: key=%s", key))
	}
	ctx := context.Background()
...
func RedisIncr(key string, delta int64) error {
	if common.DebugEnabled {
		utils.SysLog(fmt.Sprintf("Redis INCR: key=%s, delta=%d", key, delta))
	}
	// 检查键的剩余生存时间
...
func RedisHIncrBy(key, field string, delta int64) error {
	if common.DebugEnabled {
		utils.SysLog(fmt.Sprintf("Redis HINCRBY: key=%s, field=%s, delta=%d", key, field, delta))
	}
	ttlCmd := RDB.TTL(context.Background(), key)
	ttl, err := ttlCmd.Result()
	if err != nil && !errors.Is(err, redis.Nil) {
...
func RedisHSetField(key, field string, value interface{}) error {
	if common.DebugEnabled {
		utils.SysLog(fmt.Sprintf("Redis HSET field: key=%s, field=%s, value=%v", key, field, value))
	}
	ttlCmd := RDB.TTL(context.Background(), key)
	ttl, err := ttlCmd.Result()
	if err != nil && !errors.Is(err, redis.Nil) {
