...
func TestClient_ProxySearch_Passthrough(t *testing.T) {
	rawReq := []byte(`{"search_query":"hello","search_engine":"search_pro","search_intent":false,"count":2}`)
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"error":{"code":"1701","message":"rate_limited"}}`))
	}))
	defer srv.Close()
	...
	if string(respBody) != `{"error":{"code":"1701","message":"rate_limited"}}` {
